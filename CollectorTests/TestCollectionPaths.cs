...
namespace CollectorTests
{
    public class TestCollectionPaths
    {
        ...
        public void TestCustomCollectionPaths(){
            ...
            paths.Add(userProfile+sep+ "Collector_test.file");
            paths.Add(userProfile+sep+ "Collector_glob_test");
            paths.Add(userProfile+sep+ "Collector_Regex_test");
            ...
            {
                sr.WriteLine("glob\t**" + userProfile + sep + "Collector_glob_test");
                sr.WriteLine("static\t" + userProfile + sep + "Collector_test.file");
                if (Platform.IsUnixLike())
                    sr.WriteLine("regex\t^" + userProfile + sep + "Collector_[A-Za-z]{5}_test$");
                else
                    sr.WriteLine("regex\t^"+userProfile.Replace("\\", "\\\\")+sep+sep+ "Collector_[A-Za-z]{5}_test$");
            }
        ...
        public void TestCustomCollectionPathsRegExFail(){
            ...
            {
                if (Platform.IsUnixLike())
                    sr.WriteLine("regex\t^"+userProfile+sep+ "Collector_[A-Za-z]{10}_test$");
                else
                    sr.WriteLine("regex\t^"+userProfile.Replace("\\", "\\\\")+sep+sep+ "Collector_[A-Za-z]{10}_test$");
            }
