...
using Collector.archive;
using Collector.read;
using Collector.src.read;
using Renci.SshNet;
using ArchiveFile = Collector.archive.File;
...
namespace Collector
{
    internal static class Program
    {
        private static int Main(string[] args)
        {
            ...
            logger.debug(String.Format("Argument {0} is set to {1}", "UploadCleanUp", arguments.UploadCleanUp));
            logger.debug(String.Format("Argument {0} is set to {1}", "S3Bucket", arguments.S3Bucket));
            logger.debug(String.Format("Argument {0} is set to {1}", "S3AccessKey", arguments.S3AccessKey));
            logger.debug(String.Format("Argument {0} is set to {1}", "S3Region", arguments.S3Region));
            logger.debug(String.Format("Argument {0} is set to {1}", "S3Secret", "[omitted in log file]"));
            ...
            try
            {
                ...
                if (arguments.UseS3)
                {
                    ...
                    logger.debug("Start S3 Upload");
                    S3Upload(arguments, outputPath, logger);
                }
            }
            catch (Exception e)
            {
        ...
        /// the user specified <c>--no-uploadcleanup</c> at invocation.
        /// </summary>
        ...
        /// <summary>
        /// Handle the connection to the S3 server and uploading the resulting
        /// archive file. In the case the upload fails, this method will attempt
        /// to re-upload 3 times, with a 30 second pause between to allow time
        /// for the network to become more stable. If the upload is successful,
        /// the resulting archive file will be removed from the system - unless
        ...
        /// <param name="arguments">User specified arguments with SFTP and other details</param>
        /// <param name="logger">Logging object</param>
        private static void S3Upload(Arguments arguments, string outputPath, Logger logger)
        {
            ...
            int max_tries = 3;
            int num_tries = 0;
            AWSConfigs.AWSRegion = arguments.S3Region;
            while (!successfulUpload && (num_tries < max_tries))
            {
                bool attemptSuccess = false;
                try
                {
                    BasicAWSCredentials basicCredentials = new BasicAWSCredentials(arguments.S3AccessKey, arguments.S3Secret);
                    AmazonS3Client s3Client = new AmazonS3Client(basicCredentials);

                    var putFile = new PutObjectRequest
                    {
                        BucketName = arguments.S3Bucket,
                        Key = arguments.OutputFileName,
                        FilePath = outputPath,
                    };

                    PutObjectResponse response = s3Client.PutObjectAsync(putFile).Result;
                ...
                {
                    logger.warn(String.Format("Upload failed. Retrying {0} more times. " + error.Message, max_tries - num_tries));
                    logger.warn("Sleeping for 30 seconds");
                    num_tries++;
                    System.Threading.Thread.Sleep(30 * 1000);
                }
            ...
            if (!successfulUpload)
            {
                logger.warn("Unable to upload to S3. Zip file not removed. Please upload through another manner.");
            }
        ...
        private static void ArchiveCurrentLog(Logger logger, SharpZipArchive archive, IFileSystem system)
        {
            ...
            {
                string logfileTempPath = String.Format("Collection_Log_{0}.log", DateTime.UtcNow.ToString("yyyy-MM-dd_HH-mm-ss"));
